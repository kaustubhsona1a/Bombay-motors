...
import { ChevronLeft, Upload, X, Check, ShieldAlert, ArrowLeft, ArrowRight, Star } from 'lucide-react';
...
export const EditVehicle: React.FC = () => {
  ...
  // Form states initialized with empty or dummy
  ...
  useEffect(() => {
    if (vehicle) {
      ...
      setOwnerCount(vehicle.ownerCount);
      setRegistration(vehicle.registration);
      setDescription(vehicle.description);
      setInspectionNotes(vehicle.inspectionNotes);
  ...
  if (!vehicle) {
    return (
      <div className="bg-[#09090b] min-h-screen text-white flex flex-col justify-center items-center p-6 text-center">
        ...
        <h2 className="text-2xl font-sans font-bold uppercase">Listing Context Not Found</h2>
        <p className="text-zinc-500 max-w-sm mt-1 text-sm font-light">Unable to load parameters. Listing may have been deleted.</p>
        <Link to="/dealer-management/inventory" className="text-xs font-mono font-bold tracking-widest text-[#c5a059] hover:underline mt-6 uppercase">
          Back to Warehouse
        </Link>
      </div>
    );
  ...
  const handleImageUpload = async (files: FileList | null) => {
    ...
    for (let i = 0; i < files.length; i++) {
      try {
        ...
        if (isFirebaseMock) {
          // 1. Client-side canvas compression for local Storage when offline
          const compressedB64 = await compressImage(file);
          list.push(compressedB64);
        } else {
          // 2. Ultra-fast compression to JPEG Blob before storage upload
    ...
    if (list.length > 0) {
      ...
      const { optimizedImages } = enforceFirestoreSizeGuard(merged, 4000, (msg) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (images.length === 0) {
      showToast('Showroom requires at least one photograph.', 'info');
      return;
    }
    ...
    try {
      // Always enforce size guard to protect Firestore size limits, especially if using Base64 fallbacks
      const { optimizedImages } = enforceFirestoreSizeGuard(images, 4000, (msg) => {
        showToast(msg, 'warning');
      });
      let finalImages = optimizedImages;
      ...
      showToast('Listing updated successfully inside Firestore.', 'success');
      navigate('/dealer-management/inventory');
    } catch (err: any) {
      console.error('Error updating vehicle:', err);
      let errorMsg = 'Firestore security update failed.';
      if (err && err.message) {
        try {
          const parsed = JSON.parse(err.message);
          if (parsed && typeof parsed === 'object' && parsed.error) {
            errorMsg = `Update failed: ${parsed.error}`;
          } else {
  ...
  return (
    <div className="bg-[#09090b] text-[#f4f4f5] min-h-screen py-12 px-4 md:px-8 font-sans">
      <div className="max-w-4xl mx-auto">
        ...
        {/* Page Heading */}
        <div className="mb-10">
          <span className="text-[#c5a059] font-mono text-xs tracking-widest uppercase">CATALOG MODIFICATION WINDOW</span>
        ...
        <form onSubmit={handleSubmit} className="bg-[#121214] border border-white/5 rounded-2xl p-6 md:p-10 shadow-2xl flex flex-col gap-8">
          ...
          {/* Section 1 */}
          <div>
            <h3 className="text-sm font-mono tracking-widest uppercase text-[#c5a059] border-b border-white/5 pb-2 mb-6">Section 1: Specs Modifications</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {/* Model */}
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-mono text-zinc-500 uppercase tracking-widest">Model Name *</label>
                <input
                  type="text"
                  required
                  value={model}
              ...
              {/* Variant */}
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-mono text-zinc-500 uppercase tracking-widest">Variant Name *</label>
                <input
                  type="text"
                  required
                  value={variant}
                  onChange={(e) => setVariant(e.target.value)}
                  className="w-full bg-[#1c1c1f] border border-white/5 focus:border-[#c5a059]/40 rounded-xl p-3 text-sm text-white"
                />
              </div>
              ...
              {/* Transmission */}
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-mono text-[#71717a] uppercase tracking-widest">Transmission *</label>
                <select
                  value={transmission}
              ...
              {/* Fuel Type */}
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-mono text-[#71717a] uppercase tracking-widest">Fuel Engine *</label>
                <select
                  value={fuelType}
          ...
          {/* Section 2 */}
          <div>
            <h3 className="text-sm font-mono tracking-widest uppercase text-[#c5a059] border-b border-white/5 pb-2 mb-6">Section 2: Valuations Configurations</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* Price */}
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-mono text-[#71717a] uppercase tracking-widest">Valuation (INR Lakhs) *</label>
                <input
                  type="number"
                  required
                  step="0.5"
              ...
              {/* RTO Registration */}
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-mono text-[#71717a] uppercase tracking-widest">RTO Registration *</label>
                <input
                  type="text"
                  required
                  value={registration}
          ...
          {/* Section 3 */}
          <div>
            <h3 className="text-sm font-mono tracking-widest uppercase text-[#c5a059] border-b border-white/5 pb-2 mb-6">Section 3: Narratives & Diagnostics</h3>
            <div className="flex flex-col gap-6">
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-mono text-zinc-500 uppercase tracking-widest">Showroom Editorial Description</label>
                <textarea
                  rows={4}
                  value={description}
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-mono text-zinc-500 uppercase tracking-widest font-bold">Bumper inspection Report</label>
                <textarea
                  rows={3}
              ...
              {/* Status */}
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-mono text-zinc-500 uppercase tracking-widest font-bold">Listing Status Code</label>
                <select
                  value={status}
          ...
          {/* Section 4 */}
          <div>
            <h3 className="text-sm font-mono tracking-widest uppercase text-[#c5a059] border-b border-white/5 pb-2 mb-4">Section 4: Premium Accessories & features</h3>
          ...
          {/* Section 5 */}
          <div>
            <h3 className="text-sm font-mono tracking-widest uppercase text-[#c5a059] border-b border-white/5 pb-2 mb-4">Section 5: Media Catalog</h3>
            
            {/* Direct URL attachment option */}
            <div className="border border-white/5 bg-[#171719] p-4 rounded-2xl flex flex-col justify-center gap-3 mb-4">
              <div>
                <h4 className="text-xs font-mono font-bold uppercase text-[#c5a059]">Direct URL attachment</h4>
                <p className="text-[11px] text-zinc-500 leading-normal mt-1">If uploading media files fails due to size limits, paste direct HTTP/HTTPS references below (Unsplash, Imgur, CDN, etc.):</p>
              </div>
            ...
            <div 
              ...
              className="border-2 border-dashed border-white/10 hover:border-[#c5a059]/40 bg-zinc-950/40 p-8 rounded-2xl flex flex-col items-center justify-center text-center cursor-pointer transition-all duration-300"
            >
              <Upload className="w-10 h-10 text-zinc-500 mb-3" />
              <p className="text-sm text-zinc-300 font-sans font-medium">Drop photos or drag files inside frame</p>
              <p className="text-[11px] text-zinc-650 font-mono mt-1">OR CLICK TO BROWSE REPLACEMENT PHOTOGRAPHS</p>
              <input
                ref={fileInputRef}
                type="file"
                multiple
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
              {images.map((img, idx) => (
                ...
                >
                  <img src={img} alt="diagnostics thumb" className="w-full h-full object-cover pointer-events-none select-none" />
          ...
          <div className="flex justify-end gap-4 p-4 border-t border-white/5 mt-4">
            ...
            >
              {isSaving ? 'UPDATING FIRESTORE...' : 'APPLY SPEC CHANGES'}
            </button>
          </div>
